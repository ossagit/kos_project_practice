...
public class UsrArticleController {
	...
	public ResultData<Article> doAdd(HttpSession httpSession, String title, String body) {
		...
		if(isLogined == false) {
			return ResultData.from("F-A", "로그인 후 이용해주세요", null);
		}
		if(Ut.empty(title)) {
			return ResultData.from("F-1", "title을 입력해주세요", null);
		}
		if(Ut.empty(body)) {
			return ResultData.from("F-1", "body을 입력해주세요", null);
		}
	...
	public ResultData<Article> getArticle(int id) {
		...
		if ( article == null ) {
			return ResultData.from("F-1", Ut.f("%d게시물이 존재하지 않습니다", id),null);
		}
	...
	public ResultData<Integer> doModify(int id, String title, String body) {
		...
		return ResultData.from("F-1", Ut.f("%d번 게시물을 수정하였습니다", id), null);
	}
	// 액션 메서드 끝
